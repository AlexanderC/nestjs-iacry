...
import {
  IACRY_OPTIONS, SEQUELIZE_STORAGE,
  IS_ALLOWED, IS_ALLOWED_ANY, IS_ALLOWED_IMPLICIT,
} from './constants';
import {
  PolicyInterface, Action, ActionObject, Resource,
  ResourceObject, Principal, PrincipalObject, PRINCIPAL,
...
export class CoreService extends CoreHelper {
  ...
  constructor(
    @Inject(IACRY_OPTIONS) private readonly options: Options,
  ) {
    super();
      ...
      case 'string':
        switch (options.storage.toLowerCase()) {
          case SEQUELIZE_STORAGE:
            this.storage.storages.push(new SequelizeStorage(options.storageRepository));
            break;
          default: throw new BaseError(`Unrecognized  PolicyInterface Storage type: ${options.storage}`);
        }
        break;
      default: this.storage.storages.push(<PolicyStorage>options.storage);
    }
  ...
  async attach(
    ...
    const principal = isEntity(rawPrincipal)
      ...
      : <PrincipalObject>this.normalizeDynamicIdentifier(<Principal | PrincipalObject>rawPrincipal, PRINCIPAL);
  ...
  async reset(
    ...
    const principal = isEntity(rawPrincipal)
      ? <PrincipalObject>toDynamicIdentifier(rawPrincipal)
      : <PrincipalObject>this.normalizeDynamicIdentifier(<Principal | PrincipalObject>rawPrincipal, PRINCIPAL);
    return rawPolicies ? this.storage.save(principal, rawPolicies) : this.storage.purge(principal);
  }
  ...
  async retrieve(
    ...
    return rawPolicies.map(
      rawPolicy => typeof rawPolicy === 'string' ? <PolicyInterface>JSON.parse(rawPolicy) : rawPolicy
    );
  }
  ...
  async isGranted(
    ...
    switch (rule) {
      ...
      case IS_ALLOWED_IMPLICIT:
        break;
      default: throw new BaseError(`Unrecognized firewall rule: ${rule}`);
    }
