...
import {
  PolicyInterface, Effect, Action, Resource, Principal,
  ActionObject, ResourceObject, PrincipalObject,
  DynamicIdentifier, DynamicIdentifierVector,
...
export class Policy extends CoreHelper implements PolicyInterface {
  ...
  readonly Resource: DynamicIdentifier<Resource> | DynamicIdentifierVector<Resource>;
  readonly Principal: DynamicIdentifier<Principal> | DynamicIdentifierVector<Principal>;
  ...
  property(prop: string):
    ...
    if (DYNAMIC_IDENTIFIER_PROPS.includes(prop)) {
      if (!this.isDynamicVector(this[prop])) {
        if (!this.isDynamicItem(this[prop])) { // supposably an empty property
          return [];
        }
  ...
  protected normalizePolicyDynamicIdentifiers(rawPolicy: PolicyInterface): void {
    const self = this;
