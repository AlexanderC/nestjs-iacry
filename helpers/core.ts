...
export abstract class CoreHelper {
  ...
  public isDynamicVector(
    x: DynamicIdentifier<Action | Resource | Principal> | DynamicIdentifierVector<Action | Resource | Principal>)
    : x is DynamicIdentifierVector<Action | Resource | Principal> { 
    return Array.isArray(x);
  }

  public isDynamicItem(x: DynamicIdentifier<Action | Resource | Principal>)
    : x is DynamicIdentifierItem<Action | Resource | Principal> {
    return typeof x === 'object'
      && typeof (x as DynamicIdentifierItem<Action | Resource | Principal>).parse === 'function'
      && 'value' in x;
  }

  public parseDynamicIdentifier(rawPolicy: PolicyInterface | any, prop: string, value: string | ANY)
    : ActionObject | ResourceObject | PrincipalObject {
    const assignmentsVector = DYNAMIC_IDENTIFIER_PROPS_MAPPING[prop];
    let valuesVector = value.split(DELIMITER).map(x => x.trim()).map(x => x.length <= 0 ? CoreHelper.ANY : x);
    ...
    if (assignmentsVector.length < valuesVector.length) {
      ...
      valuesVector = [
        ...valuesVector,
        ...new Array(assignmentsVector.length - valuesVector.length).fill(CoreHelper.ANY),
      ];
    }

    const identifierObject = <ActionObject | ResourceObject | PrincipalObject>{};
  ...
  public normalizeDynamicIdentifier(
    thing: Resource | ResourceObject | Action | ActionObject | Principal | PrincipalObject,
    prop: string,
  ): ActionObject | ResourceObject | PrincipalObject {
    if (typeof thing === 'string') {
      return this.parseDynamicIdentifier(
        { [prop]: thing },
        prop,
        thing,
      );
