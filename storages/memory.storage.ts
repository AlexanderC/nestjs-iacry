...
export class MemoryStorage implements PolicyStorage {
  ...
  async save(_principal: PrincipalObject, rawPolicies: Array<string | PolicyInterface>): Promise<number> {
    this.policies = rawPolicies;

    return rawPolicies.length;
  }

  async add(_principal: PrincipalObject, rawPolicies: Array<string | PolicyInterface>): Promise<number> {
