...
export class SequelizeStorage implements PolicyStorage {
  ...
  async save(principal: PrincipalObject, rawPolicies: Array<string | PolicyInterface>): Promise<number> {
    return this.repository.savePrincipalPolicies(principal, rawPolicies, /*attach =*/ false);
  }
  ...
  get repository(): Storage {
    if (!this.isStorageModel(this.storageRepository)) {
      throw new TypeError('Policies repository must extend Sequelize PoliciesStorage model');
    }
