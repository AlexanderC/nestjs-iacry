...
import { ENTITY_FIELD, ID_FIELD, ResourceObject, PrincipalObject, Resource, Principal, DELIMITER } from '../interfaces/policy';
...
export function isEntity(target: any): target is Entity {
  return target
    ...
    && typeof Reflect.getMetadata(ENTITY_META_FIELD, target.constructor) === 'function'
    && typeof Reflect.getMetadata(ID_META_FIELD, target.constructor) === 'function';
}

export function toDynamicIdentifier(target: object): ResourceObject | PrincipalObject {
  if (!isEntity(target)) {
    throw new DecoratorError('Target object should use @Entity() decorator');
  }

  return <ResourceObject | PrincipalObject>{
    [ENTITY_FIELD]: Reflect.getMetadata(ENTITY_META_FIELD, target.constructor)(target),
...
export function Entity(options?: EntityOptions): ClassDecorator {
  return (target: Function) => {
    if ((!options || (!options.name && !options.nameField)) && !target.name) {
      throw new DecoratorError('Entity has no explicit name, nameField nor constructor defined');
    }

    Reflect.defineMetadata(
      ENTITY_META_FIELD,
      (instance: object) => (options && options.nameField)
        ? instance[options.nameField]
        : ((options && options.name) ? options.name : target.name.toLowerCase()),
      target,
    );
    Reflect.defineMetadata(
      ID_META_FIELD,
      (instance: object) => (options && options.idField)
