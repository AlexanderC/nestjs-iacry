...
export const extractDynamicIdentifier = dynamicIdentifierExtractor<Action>(ACTION_META_FIELD);
...
export function Action(action?: Action): MethodDecorator {
  return (
    ...
    if (!action) {
      action = `${target.constructor.name.toLowerCase()}${DELIMITER}${String(key)}`;
    }
